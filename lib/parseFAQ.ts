...
export function parseResponseWithFAQ(response: string): ParsedResponse {
  ...
  const faqPatterns = [
    /Here are some (?:general )?FAQs?.*?:\s*$/im,
    /(?:Some )?FAQs?\s*to get (?:us|you) started:\s*$/im,
    /Frequently Asked Questions?:\s*$/im,
    /Common Questions?:\s*$/im,
    /Questions?\s*and\s*Answers?:\s*$/im,
    /Here are some (?:common|helpful) (?:questions|topics).*?:\s*$/im,
    /Let me share some (?:relevant|important) (?:questions|information).*?:\s*$/im,
    /Below are some (?:key|essential) (?:questions|points).*?:\s*$/im,
    /I'll provide some (?:useful|relevant) (?:information|guidance).*?:\s*$/im,
  ];
  ...
  if (!faqSectionFound) {
    console.log(
      "🔄 No explicit FAQ section, searching for FAQ-like content..."
    );
...
function forceExtractAllFAQs(text: string): FAQItem[] {
  ...
  for (
    let patternIndex = 0;
    patternIndex < guaranteedPatterns.length;
    patternIndex++
  ) {
    ...
    console.log(
      `🔍 Trying guaranteed pattern ${patternIndex + 1}:`,
      pattern.source
    );
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      if (question && answer && question.length > 3 && answer.length > 3) {
        ...
        const isDuplicate = faqs.some(
          (faq) => normalizeText(faq.question) === normalizeText(question)
        );

        if (!isDuplicate) {
          faqs.push({ question, answer });
          console.log(
            `✅ FAQ ${faqs.length} extracted (Pattern ${
              patternIndex + 1
            }): Q: "${question}" A: "${answer.substring(0, 100)}..."`
          );
        } else {
  ...
  faqs.forEach((faq, index) => {
    console.log(
      `📌 FAQ ${index + 1}: "${faq.question}" -> "${faq.answer.substring(
        0,
        50
      )}..."`
    );
...
function extractByManualSplitting(text: string): FAQItem[] {
  ...
  console.log(
    `📋 Found ${sections.length} sections after splitting by asterisks`
  );
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    if (answerLines.length > 0) {
      ...
      console.log(
        `✅ Manual FAQ extracted: Q: "${question}" A: "${answer.substring(
          0,
          50
        )}..."`
      );
...
function parseAdvancedFAQs(faqText: string): FAQItem[] {
  ...
  for (let i = 0; i < advancedPatterns.length; i++) {
    ...
    while ((match = pattern.exec(faqText)) !== null) {
      ...
      if (i === 2) {
        // Pattern 3 with bold topics
        question = `What to do in case of ${cleanAdvancedText(match[1])}?`;
        answer = match[2] ? cleanAdvancedText(match[2]) : "";
      } else if (i === 5) {
        // Pattern 6 with multiple groups
        question = `${cleanAdvancedText(match[1])} ${cleanAdvancedText(
          match[2]
        )}?`;
      ...
      if (question && answer && question.length > 3 && answer.length > 5) {
        ...
        if (!isDuplicate) {
          ...
          console.log(
            `✅ FAQ added (Pattern ${
              i + 1
            }): Q: "${question}" A: "${answer.substring(0, 50)}..."`
          );
        } else {
          console.log(`⚠️ Duplicate FAQ skipped: "${question}"`);
        }
