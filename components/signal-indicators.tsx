...
export const SignalIndicators: React.FC<SignalIndicatorsProps> = ({
  ...
  return (
    ...
    >
      ...
      <div
        className="flex items-center gap-1 sm:gap-1.5 bg-white px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg shadow-sm border"
        title={`Network: ${deviceInfo.networkType.toUpperCase()}, Speed: ${
          deviceInfo.connectionSpeed
        }`}
      >
        {deviceInfo.onlineStatus ? (
          <>
            <Wifi
              className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${getNetworkColor(
                signalStrength
              )}`}
            />
              ...
              {[1, 2, 3, 4].map((bar) => (
                <div
                  ...
                  className={`w-0.5 bg-current rounded-full transition-colors ${
                    bar <= signalStrength
                      ? getNetworkColor(signalStrength)
                      : "text-slate-300"
                  }`}
      ...
      >
        <Battery
          className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${getBatteryColor(
