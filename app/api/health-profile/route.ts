...
export async function POST(request: NextRequest) {
  try {
    ...
    const bmi = parseFloat(
      (body.weight / (heightInMeters * heightInMeters)).toFixed(2)
    );
    ...
    const externalResponse = await fetch(
      `${process.env.HEALTH_PROFILE_API_URL}/health-profile`,
      {
        method: "POST",
        headers: {
    ...
    if (!externalResponse.ok) {
      ...
      const basicResponse = {
        ...
        disclaimer:
          "This analysis is for informational purposes only. Always consult with healthcare professionals for medical advice.",
